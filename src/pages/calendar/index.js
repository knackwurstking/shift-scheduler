 ...
 * @typedef {import("../../types/.index").DatePickerStore} DatePickerStore
 * @typedef {import("../../types/.index").WeekStartStore} WeekStartStore
 * @typedef {import("../../types/.index").LangStore} LangStore
 * @typedef {import("../../types/.index").DBDataEntry} DBDataEntry
 * @typedef {import("../../types/.index").EditModeStore} EditModeStore
 */
...
const templateDaysRow = html`
  <ui-flex-grid-row class="days-row" gap="0.05rem">
    ${templateDayItem} ${templateDayItem} ${templateDayItem} ${templateDayItem}
    ${templateDayItem} ${templateDayItem} ${templateDayItem}
  </ui-flex-grid-row>
`;

const templateItemContent = html`
  <ui-flex-grid class="content" gap="0.05rem">
    ${templateWeekDaysRow} ${templateDaysRow} ${templateDaysRow}
    ${templateDaysRow} ${templateDaysRow} ${templateDaysRow} ${templateDaysRow}
  </ui-flex-grid>
`;
...
export class CalendarPage extends UIStackLayoutPage {
  ...
  connectedCallback() {
    ...
    this.cleanup.add(
      ...
      this.#store.ui.on("date-picker", this.onDatePicker.bind(this), true),
    );
  ...
  async updateCalendarItem(current, calendarItem) {
    // {{{
    let dataEntries = await utils.getArray(
      current.getFullYear(),
      current.getMonth(),
      this.#store,
    );
  ...
  async updateWeekDays(weekStart) {
    ...
    const items = [
      ...this.shadowRoot.querySelectorAll(".week-days-row .week-day-item"),
    ];
  ...
  async onDayItemClick(target, child) {
    ...
    if (!!open) {
      ...
      this.updateDayItem(target, {
        ...dbEntry,
        shift: dbEntry.shift || rhythmStift,
      });
      return;
    }
    ...
    for (let i = 0; i < shifts.length; i++) {
      ...
      card.onclick = async () => {
        ...
        if (card.hasAttribute("active")) {
          ...
          this.#store.ui.update("edit-mode", (data) => ({
            ...data,
            active: null,
          }));
          return;
